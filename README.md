...
This package is structured so `app.py`, `requirements.txt`, `src/`, and
`inputs/` are all at the repository root.

1. Upload the contents of this folder to the root of a GitHub repository.
2. In Streamlit Community Cloud, create an app with `app.py` as the entrypoint.
3. In Advanced settings, select Python 3.12.
4. Deploy the app.
5. Use `inputs/*.csv` as the initial dummy upload set.

Before deployment, a remote terminal can validate the package with:
...
Version 1.1.0 keeps `src/slo/__init__.py` free of eager submodule imports. This
prevents the `_frozen_importlib._DeadlockError` that can occur on Streamlit
Community Cloud when `app.py` imports `slo.io` and the package initializer
simultaneously imports `slo.pipeline`.
